...
def detection(output, img, img_metas):
    ...
    print("config_height_width",width,height)
    print("img_height,width",img.shape)
    ax = get_ax(1)
    ...
    for j in range(N):
        ...
        mask =cv2.resize(mask,(x2 - x1, y2 - y1)) #resize as bbox
        mask = np.where(mask >= 0.1, True, False) #二值化 .astype(np.uint8)
        full_mask = np.zeros((height,width),dtype=np.bool)
        full_mask[y1:y2, x1:x2] = mask
        img_to_draw = np.copy(masked_image)
        print(full_mask.shape)
        if True in mask:
            print("Yes")
        print(color)
        
        img_to_draw[full_mask] = np.multiply(255,color) 
        
        masked_image = img_to_draw *0.7 + masked_image* 0.3
